...
fn load_congested_scenario_parses_without_error() {
    ...
    let result = scenarios::load_scenario(path);
    assert!(result.is_ok(), "congested.json failed to load: {:?}", result);
}
...
fn all_bundled_scenarios_have_required_fields() {
    ...
    for name in &names {
        ...
        assert!(!scenario.scenario.is_empty(), "{}: scenario name empty", name);
        assert!(!scenario.fee_stats.last_ledger.is_empty(), "{}: last_ledger empty", name);
        assert!(!scenario.fee_stats.fee_charged.p50.is_empty(), "{}: fee_charged.p50 empty", name);
        assert!(!scenario.fee_stats.max_fee.p50.is_empty(), "{}: max_fee.p50 empty", name);
    }
}
